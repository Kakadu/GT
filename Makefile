...
PLUGINS=reify gmap show compare eq foldl foldr eval stateful gfmt html hash #show_typed #htmlTy
add_plugins:
	$(eval OBPARAMS  += -I common)
...
doc:
	$(OB) GT.docdir/index.html -Is common,plugins,src #-hide Show_typed
...
REGRES_CASES += 000 001 002 004 005 006 007 008 009 010 011 012 015 016 017 018 019 020 022 023 029
REGRES_CASES += 030 031 032 036 037 041 #005
#mutual cases
REGRES_CASES += #003 013 014 082 089
REGRES_CASES += 024 025 021 040
REGRES_CASES += 081 083 086 087 090 091
REGRES_CASES += 042
# PPX based tests
REGRES_CASES += 816 803 804 805 806 810 811 812 815 820 821 822 823 824 828 #808 814
#mutual cases
